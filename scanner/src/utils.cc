...
int JudgeDelimiter(const std::string &str, int &lpoint, std::string &out) {
    for (int i = 0; i < DELIMITER_NUMBER; i++) {
        if (str[lpoint] == DELIMITER_TABLE[i][0]) { // first check first char
            if ((int)DELIMITER_TABLE[i].size() == 2 && (int)str.size() >= 2 && str[lpoint + 1] == '=') {lpoint += 2;out = DELIMITER_TABLE[i];return i;}
            else if ((int)DELIMITER_TABLE[i].size() == 1) {lpoint += 1;out = DELIMITER_TABLE[i];return i;}
        }
    }
    return -1;
}
    ...
    while (lpoint < rpoint) {
        ...
        while (lpoint < rpoint) {
            ...
            if (currentState == -1) Error(lineNumberContent);
                ...
                Token token;
                switch(currentState) {
                    case 10: {ProcessNumber(word, token, consts); break;}
                    case 11: {ProcessStr(word, token, variables); break;}
                    case 12: {}
                    case 13: {ProcessDelimiter(word, delimiterId, token); break;}
                    case 14: {}
...
void ProcessNumber(const std::string word, Token &token, std::vector<PDouble> &consts) {
    ...
    if (std::find(consts.begin(), consts.end(), baseNumber) == consts.end()) token.code = DOUBLE_CODE, token.value = Token::sizeConst++, consts.push_back(PDouble(baseNumber));
...
void ProcessStr(const std::string word, Token &token, std::vector<std::string> &variables) {
    ...
    if (it != KEYWORDS_TABLE.end()) token.code=KEYWORD_CODE, token.value = it-KEYWORDS_TABLE.begin();
    else if (std::find(variables.begin(), variables.end(), word) == variables.end()) {
        token.code = VAR_CODE, token.value = Token::sizeVariable++, variables.push_back(word);
    }
}
